...
class TambahKamar extends Component {
    ...
    request({
      ...
      if (this.props.params.id >= 0) {
        ...
        this.setState({
          ...
          harga_kamar       : (this.props.params.data.harga_kamar) ? formatCur({value:this.props.params.data.harga_kamar.split('.').join(''), input:true}) : '',
          gambar_link       : this.props.params.data.gambar_link,
          value_kamar       : this.props.params.data.value_kamar,
          value_lantai      : this.props.params.data.value_lantai,
          lantai            : this.props.params.data.lantai,
          tipe_kamar        : this.props.params.data.tipe_kamar,
          data_kamar        : this.props.params.data.data_kamar,
          kamar_id          : this.props.params.data.kamar_id,
          status            : this.props.params.data.status,
          value_status      : this.state.master_status[this.props.params.data.status.row],
          data_tipe_kamar   : res.data.data_tipe_kamar,
          fasilitas_kamar   : data
        });
      } else {
        res.data.fasilitas_kamar.map((item, index) => {
          data.push({
            master_fasilitas_kamar_id : item.master_fasilitas_kamar_id,
            fasilitas_kamar           : item.fasilitas_kamar,
            check                     : false
        ...
        this.setState({
          data_tipe_kamar   : res.data.data_tipe_kamar,
          session_upload_id : res.data.session_upload_id,
          status            : {'row' : 1},
          fasilitas_kamar   : data
        });
      }
      for (var i = 1; i <= this.props.params.jumlah_lantai; i++) {
  ...
  _onSubmit = () => {
    ...
    this.state.fasilitas_kamar.map((item, index) => {
      if (item.master_fasilitas_kamar_id != '0' && item.check) {
        data.push(item);
        nama_fasilitas.push(item.fasilitas_kamar);
      }
    });
    this.props.params?.data_kamar.map((item, index) => {
      if (item.nomor_kamar == this.state.nomor_kamar) {
        is_kamar = true;
      }
    })
    this.setState({
      ...
      errNomorKamar  : (this.state.nomor_kamar && !is_kamar ? false : true),
      ...
      if (
        ...
        let params = {
          ...
          'kamar_id'          : (this.state.kamar_id) ? this.state.kamar_id : null
        }
        this.props.params.action(params, this.props.params.id, this.props.params.data, this.state.value_lantai);
      }
  ...
  _onChangeTipeKamar = (select) => {
    this.setState({ tipe_kamar : select, value_kamar : this.state.data_tipe_kamar[select.row].tipe_kamar });
  }

  _onChangeLantai = (select) => {
    this.setState({ lantai : select, value_lantai : this.state.jumlah_lantai[select.row] });
  }

  _onChangeStatus = (select) => {
    this.setState({ status : select, value_status : this.state.master_status[select.row] });
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(TambahKamar);
